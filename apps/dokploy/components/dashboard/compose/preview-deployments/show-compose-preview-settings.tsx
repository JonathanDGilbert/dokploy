...
const schema = z
	.object({
		...
		port: z.number(),
		previewServiceName: z.string().min(1),
...
export const ShowComposePreviewSettings = ({ composeId }: Props) => {
	const [isOpen, setIsOpen] = useState(false);
	const [isEnabled, setIsEnabled] = useState(false);
	const { mutateAsync: updateCompose, isPending } = api.compose.update.useMutation();
	...
	useEffect(() => {
		if (data) {
			form.reset({
				...
				previewServiceName: data.previewServiceName || "gateway",
	...
	const onSubmit = async (formData: Schema) => {
		updateCompose({
			...
			previewServiceName: formData.previewServiceName,
		...
		<div>
			<Dialog open={isOpen} onOpenChange={setIsOpen}>
				...
				<DialogContent className="sm:max-w-5xl w-full">
					<DialogHeader>
						<DialogTitle>Compose Preview Deployment Settings</DialogTitle>
						<DialogDescription>
							Configure PR preview stacks: hostname pattern, Traefik target
							service, ports, and extra environment for compose up.
						</DialogDescription>
					</DialogHeader>
							...
							>
								<div className="grid gap-4 lg:grid-cols-2">
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormDescription>
													Internal port for the Traefik backend (compose service
													port).
												</FormDescription>
												<FormMessage />
											</FormItem>
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												<FormLabel>Public service name</FormLabel>
												<FormControl>
													<Input placeholder="gateway" {...field} />
												</FormControl>
												<FormDescription>
													Compose service that receives traffic for the preview
													hostname (Traefik labels are applied to this service).
												</FormDescription>
												<FormMessage />
											</FormItem>
										)}
